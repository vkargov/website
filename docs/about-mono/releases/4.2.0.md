...
Cross-process Shared Handles
----------------------------

They are now disabled by default; this will allow us to remove them
in a future version. You can still enable them at compilation time
by passing `--enable-shared-handles`.
